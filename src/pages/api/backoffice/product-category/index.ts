...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (method === "GET") {
    ...
    const {name, isAvailable, companyId, isArchived} = req.body;
    const isValid = name && isAvailable !== undefined && companyId;
    if (!isValid) return res.status(400).send("Bad request");
    const productCategory = await prisma.productCategory.create({
      data: {name, isAvailable, companyId, isArchived},
    });
    ...
    await prisma.productCategory.update({data: {isArchived: true}, where: {id}});
